...
현장 설비(PLC·센서·계측기)에서 초 단위로 유입되는 IoT 포인트 값을, 여러 테넌트의 화면·알람·리포트에 동시에 전달해야 했습니다. 디바이스 3,501대·태그 3만 개 규모, 초당 9,000건 처리 용량이 대상이었고, 처음 이 구조를 설계할 때 가장 먼저 부딪힌 질문은 "값을 어디까지 DB에 즉시 기록해야 하는가"였습니다.
